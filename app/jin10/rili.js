...
socket.on('today', function(msg){
    if(msg) {
        ...
        let all_source_id = [];
        for(let m of msg) {
            all_source_id.push(m['id']);
        }

        console.log(msg);
        const today = moment().format("YYYY-MM-DD");
        const delete_sql = `delete from crawl_economic_calendar where date(pub_time)='${today}' and source_id not in (${all_source_id.join(',')});`;
        console.log(delete_sql);
        mysqlconnection.query(delete_sql, function(err, rows, fields) {
            if (err) {
                logger.error('error sql: ' + sql);
                logger.error(err);
            }
            else{
                logger.info('删除数据成功, ' + sql);
            }
        });
    }
});
